...
def simulate_single_iteration(filtered_fixtures_np: pd.DataFrame, standings: pd.DataFrame) -> pd.DataFrame:
    ...
    t4points_threshold = sorted(standings['final_standings'], reverse=True)[4]
    t2points_threshold = sorted(standings['final_standings'], reverse=True)[2]

    standings['T2'] = (standings['final_standings'] > t2points_threshold).astype(int)
    standings['T2NRR'] = (standings['final_standings'] == t2points_threshold).astype(int)

    standings['Q'] = (standings['final_standings'] > t4points_threshold).astype(int)
